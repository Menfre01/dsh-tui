...
- **Session list**: `←` opens/closes when idle (↑↓ navigate, Enter switch, Esc close); aligned with dsh web visibility rules (only the current blank session visible, subagent sessions hidden), shows host title projection, fixed-height window (8 items + scroll follow + more hints), blank line between items, selected style consistent with other overlays (left accent border + green bold), second line shows the working directory
