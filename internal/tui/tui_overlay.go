...
func questionOtherKeyBindings(lc *Messages) []key.Binding {
	return []key.Binding{
		key.NewBinding(key.WithKeys("enter"), key.WithHelp("Enter", lc.KeyConfirm)),
		key.NewBinding(key.WithKeys("e"), key.WithHelp("E", lc.PickerSelectEffort)),
...
func (m *model) renderModelPickerOverlay(boxWidth int) string {
	...
	if m.effortPickerMode {
		// effort 面板:当前模型的思考档位列表(web 的 Effort pane)
		height := m.effortCount()
		if height > 5 {
			height = 5
		}
		if height < 1 {
		...
		modelName := ""
		if mi := m.effortPickerModelIdx; mi >= 0 && mi < len(m.modelPickerItems) {
			modelName = m.modelPickerItems[mi].Name
		}
		title := styleOverlayTitle.Width(innerWidth).Render(
			fmt.Sprintf("%s %s", m.msg().PickerSelectEffort, modelName))
		contentLines := []string{title, ""}
		contentLines = append(contentLines, styleOverlayBody.Width(innerWidth).Render(m.effortPickerList.View()))
		contentLines = append(contentLines, "")
		contentLines = append(contentLines, styleOverlayBody.Render(
			"[↑/↓] "+m.msg().KeyNav+"    [Enter] "+m.msg().KeyConfirm+"    [Esc] "+m.msg().KeyBack))
		return renderOverlayBox(boxWidth, m.overlayAnimFrame, strings.Join(contentLines, "\n"))
	}
