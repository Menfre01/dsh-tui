...
func (m *model) scanFilesAsync() {
	...
	go func() {
		...
		items := doScanFilesWithContext(ctx, m.cwd, filter)
		if m.program != nil {
...
func doScanFilesWithContext(ctx context.Context, cwd, filter string) []pickerItem {
	filter = resolveTilde(filter)

	if filepath.IsAbs(filter) {
		return doScanAbsolute(ctx, cwd, filter)
	}
	return doScanRelative(ctx, cwd, filter)
}

// doScanRelative 相对路径扫描:基于 cwd,深度扫描项目内部,浅层列出父目录兄弟。
func doScanRelative(ctx context.Context, cwd, filter string) []pickerItem {
...
func doScanAbsolute(ctx context.Context, cwd, filter string) []pickerItem {
	ctx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
